...
def extract_results(
    ...
    Returns:
        membership_df: Node-level assignments
    ...
    summary_df = summary_df.sort_values(by="total_degree", ascending=False)
...
def plot_layered_matrix(
    ...
    Nodes are sorted by community to reveal block structure.
    """
    import matplotlib.pyplot as plt

    print(f"  Generating 4-layer matrix visualization...")

    # Get node ordering by community
    b = state.get_bs()[0]  # Finest level community assignment
    node_order = sorted(range(g.num_vertices()), key=lambda i: b[i])
    order_map = {original: new for new, original in enumerate(node_order)}

    N = g.num_vertices()

    # Compute community boundaries for grid lines
    sorted_b = [b[i] for i in node_order]
    boundaries = []
    for i in range(1, N):
    ...
    for idx, (ax, cmap, layer_title) in enumerate(zip(axes.flat, cmaps, titles)):
        ...
        im = ax.imshow(matrices[idx], cmap=cmap_copy, interpolation='nearest', aspect='equal')
        ax.set_title(layer_title, fontsize=11, fontweight='bold')
        ax.set_xlabel("Target Node", fontsize=9)
        if idx % 2 == 0:
            ax.set_ylabel("Source Node", fontsize=9)
        ...
        for pos in boundaries:
            ax.axhline(pos - 0.5, color='black', linewidth=0.3, alpha=0.5)
            ax.axvline(pos - 0.5, color='black', linewidth=0.3, alpha=0.5)
...
def plot_results(
    ...
    try:
        plot_layered_matrix(state, g, matrix_path)
    except Exception as e:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--save-all-levels",
        action="store_true",
        help="Save all hierarchical levels to separate files",
    )
    parser.add_argument(
        "--plot",
        action="store_true",
        help="Generate visualization plots (requires graphviz)",
    )
...
def main() -> None:
    ...
    membership_df, summary_df = extract_results(state, g, id_map, node_map, level=0)
    ...
    if args.plot:
        ...
        plot_results(state, g, output_dir)
